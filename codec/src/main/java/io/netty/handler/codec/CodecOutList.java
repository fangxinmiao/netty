 ...
 */
final class CodecOutList extends AbstractList<Object> implements RandomAccess {

    private static final Recycler<CodecOutList> RECYCLER = new Recycler<CodecOutList>() {
        @Override
        protected CodecOutList newObject(Handle<CodecOutList> handle) {
            return new CodecOutList(handle);
        }
    };

    static CodecOutList newInstance() {
        return RECYCLER.get();
    }

    private final Recycler.Handle<CodecOutList> handle;
    private int size;
    // Size of 16 should be good enough for 99 % of all users.
    private Object[] array = new Object[16];
    private boolean insertSinceRecycled;

    private CodecOutList(Recycler.Handle<CodecOutList> handle) {
        this.handle = handle;
    }
